...
#pragma warning(pop)
	...
	namespace events
	{
		...
		struct Event
		{
			...
			EventType m_type = EventType::Unkown;

			void SetValues(int a_valueA, int a_valueB)
			{
				m_values[0] = a_valueA;
				m_values[1] = a_valueB;
				m_isSet = true;
			}
			void SetValues(float a_valueA, float a_valueB)
			{
				m_values[0] = unsigned int (a_valueA);
				m_values[1] = unsigned int(a_valueB);
				m_isSet = true;
			}

			bool IsSet() { return m_isSet; }
			unsigned int GetValueA() { return m_values[0]; }
			unsigned int GetValueB() { return m_values[1]; }
