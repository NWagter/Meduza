...
void Me::Renderer::Dx12::Context::LogAdapters() const
{
	uint16_t counter = 0u;
	IDXGIAdapter* adapter = nullptr;
	std::vector<IDXGIAdapter*> adapterList;
	ME_GFX_LOG("---------------------------------------------------------------- \n"); 
	printf("Found Graphic Adapters \n");
	while (m_device->GetFactory()->EnumAdapters(counter, &adapter) != DXGI_ERROR_NOT_FOUND)
	{
		...
		std::string log(text.begin(), text.end());
		printf(log.c_str());
